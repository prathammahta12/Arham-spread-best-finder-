...
# --- SUPABASE REST CONFIG (EXACT PROJECT ID) ---
SUPABASE_URL = "https://pniglxgqdftajqkmuouf.supabase.co"
      ...
      if user_data is None:
        ...
        if user["password"] != u_pass:
          ...
          st.warning(
            ...
            and datetime.strptime(user["valid_until"], "%Y-%m-%d").date()
            < date.today()
        ):
      ...
      if res.status_code == 200:
        for u in res.json():
          if u["username"] == "admin":
            continue
          with st.expander(
              f"User: {u['username']} | Phone: {u.get('phone')} | Status:"
              f" {'✅ Approved' if u.get('is_approved') else '⏳ Pending'}"
          ):
            ...
            with col2:
              if st.button("Approve / Grant Access", key=f"app_{u['id']}"):
                exp_date = (date.today() + timedelta(days=days)).strftime(
                    "%Y-%m-%d"
                )
                requests.patch(
